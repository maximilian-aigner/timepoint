...
tpmodel.fit.poisson <- function(eventlist, covariates, ilink) {
  ...
  result <- stats::optim(th0, fn = \(x) -.lhood.unmarked(nt, bds, covariates, x), control = list(trace = 1),
...
.lhood.unmarked <- function(ntimes, bds, fs, th) {
  th <- as.matrix(th)
...
.lhood.marked <- function(ntimes, xs, bds, fs, th) {
  th <- as.matrix(th)
  print(dim(th))
